  ...
  return (
    <div className={`space-y-8 ${mode === "pdf" ? "print:space-y-4" : ""}`}>
      ...
      <div className="bg-white p-8 rounded-lg shadow-sm page-break">
        <div className="text-center py-16 rounded-lg text-white" style={{ backgroundColor: data.settings.headerColor }}>
          ...
          <div className="bg-white/20 p-6 rounded-lg inline-block">
            <h2 className="text-2xl font-semibold mb-2">{data.patientInfo.name}</h2>
            <p className="text-lg">Sample Code: {data.patientInfo.sampleCode}</p>
            <p className="text-lg">Report Date: {data.patientInfo.reportDate}</p>
          </div>
        </div>
      </div>
      ...
      {/* Page 2: Disclaimer */}
      <div className="bg-white p-8 rounded-lg shadow-sm page-break">
        <h2 className="text-2xl font-bold mb-6 text-red-600">⚠️ DISCLAIMER</h2>
        <div className="prose max-w-none">
          <p className="text-gray-700 leading-relaxed whitespace-pre-line">{data.content.disclaimer}</p>
        </div>
      </div>

      {/* Page 3: Welcome Letter */}
      <div className="bg-white p-8 rounded-lg shadow-sm page-break">
        <h2 className="text-2xl font-bold mb-6" style={{ color: data.settings.headerColor }}>
          🧬 Welcome to Your Genomics Journey
        </h2>
        <div className="prose max-w-none">
          <p className="text-gray-700 leading-relaxed whitespace-pre-line mb-6">{data.content.introduction}</p>
          <div className="bg-blue-50 p-6 rounded-lg">
            <h3 className="text-lg font-semibold mb-3">What is Genomics?</h3>
            <p className="text-gray-700 leading-relaxed whitespace-pre-line">{data.content.genomicsExplanation}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white p-8 rounded-lg shadow-sm page-break">
        ...
        <div className="mb-8">
          ...
          <div className="grid md:grid-cols-3 gap-4">
            {Object.entries(data.dietAnalysis.macronutrients).map(([key, item]) => (
              <Card key={key}>
                ...
                <CardContent>
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-2xl font-bold">{item.score}/10</span>
                    <Badge style={{ backgroundColor: getScoreColor(item.score) }} className="text-white">
        ...
        <div className="mb-8">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {Object.entries(data.dietAnalysis.foodSensitivities).map(([key, item]) => (
              <Card key={key}>
                <CardHeader className="pb-3">
                  <CardTitle className="text-sm">
                    {key.replace(/([A-Z])/g, " $1").replace(/^./, (str) => str.toUpperCase())}
              ...
              <Card key={key}>
                ...
                <CardContent>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <Badge style={{ backgroundColor: getScoreColor(item.score) }} className="text-white text-xs">
                      {item.level || getScoreLevel(item.score)}
                    </Badge>
                  </div>
                  <Progress value={item.score * 10} className="mb-2" />
                  <p className="text-xs text-gray-600 mb-2">{item.healthImpact}</p>
      ...
      <div className="bg-white p-8 rounded-lg shadow-sm page-break">
        ...
        <div className="mb-8">
          <h3 className="text-xl font-semibold mb-4">Exercise Type Suitability</h3>
          <div className="grid md:grid-cols-2 gap-4">
            {Object.entries(data.sportsAndFitness.exerciseType).map(([key, item]) => (
              <Card key={key}>
                <CardHeader className="pb-3">
                  <CardTitle className="text-sm">
                    {key.replace(/([A-Z])/g, " $1").replace(/^./, (str) => str.toUpperCase())}
        ...
        <div className="mb-8">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {Object.entries(data.sportsAndFitness.performance).map(([key, item]) => (
              <Card key={key}>
                <CardHeader className="pb-3">
                  <CardTitle className="text-sm">
                    {key.replace(/([A-Z])/g, " $1").replace(/^./, (str) => str.toUpperCase())}
        ...
        <div className="mb-8">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {Object.entries(data.sportsAndFitness.injuryManagement).map(([key, item]) => (
              <Card key={key}>
                <CardHeader className="pb-3">
                  <CardTitle className="text-sm">
                    {key.replace(/([A-Z])/g, " $1").replace(/^./, (str) => str.toUpperCase())}
