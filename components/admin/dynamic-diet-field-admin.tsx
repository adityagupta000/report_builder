...
import { FieldInput, FieldTextarea, FieldSelect } from "@/components/admin/dynamic-field-helpers"
...
interface DynamicDietFieldAdminProps {
  ...
  updateDynamicDietFieldDefinitions: (definitions: DynamicDietFieldDefinition[]) => void
  onSave: () => void
  onReset: () => void
}
...
export default function DynamicDietFieldAdmin({
  ...
  const [localFields, setLocalFields] = useState<DynamicDietFieldDefinition[]>(dynamicDietFieldDefinitions)
  ...
  const [newFieldData, setNewFieldData] = useState<Omit<DynamicDietFieldDefinition, "_uuid" | "id">>({
    label: "",
  ...
  const updateLocalField = (index: number, key: keyof DynamicDietFieldDefinition, value: any) => {
    ...
    if (key === "id") {
      const newId = String(value)
        ...
        .replace(/\s+/g, "_")
        .replace(/[^a-z0-9_]/g, "")
      const isDuplicate = localFields.some((f, i) => i !== index && f.id === newId)
      if (isDuplicate) {
        toast({
          title: "Validation Error",
  ...
  const deleteField = (idToDelete: string) => {
    ...
    const updatedFields = localFields.filter((field) => field.id !== idToDelete)
    setLocalFields(updatedFields)
    updateDynamicDietFieldDefinitions(updatedFields)
    toast({
      title: "Field Deleted",
  ...
  return (
    <Card className="shadow-lg border-0 bg-white">
      <CardHeader className="bg-gradient-to-r from-amber-500 to-orange-600 text-white rounded-t-lg">
        <CardTitle className="text-2xl">📊 Dynamic Diet Field Management</CardTitle>
      ...
      <CardContent className="space-y-8 p-8">
        ...
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-xl font-semibold text-gray-800">Configured Fields</h3>
        ...
        {localFields.length === 0 ? (
          <div className="text-center py-12 text-gray-500 italic">No dynamic diet fields defined yet.</div>
        ) : (
              ...
              {(provided) => (
                <div {...provided.droppableProps} ref={provided.innerRef} className="space-y-4">
                  {localFields.map((field, index) => (
                    <Draggable key={field._uuid} draggableId={field._uuid} index={index}>
                      {(provided, snapshot) => (
                        <div
                          ref={provided.innerRef}
                          {...provided.draggableProps}
                          className={`border rounded-lg p-4 bg-white shadow-sm transition-all duration-200 ${
                            snapshot.isDragging ? "bg-gray-100 ring-2 ring-blue-500 shadow-lg" : "hover:shadow-md"
                          }`}
                        >
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            <FieldInput
                              ...
                              value={field.id}
                              onChange={(val) => updateLocalField(index, "id", val)}
                              disabled // ID is derived from label, not directly editable
                            />
                            <FieldInput
                              label="Label"
                              value={field.label}
                              onChange={(val) => updateLocalField(index, "label", val)}
                            />
                            <FieldSelect
                              label="Category"
                              value={field.category}
                              options={categories}
                              onChange={(val) => updateLocalField(index, "category", val)}
                            />
                            <FieldInput
                              label="Min Score"
                              type="number"
                              value={field.min}
                              onChange={(val) => updateLocalField(index, "min", val)}
                              min={0}
                              max={field.max}
                            />
                            <FieldInput
                              ...
                              type="number"
                              value={field.max}
                              onChange={(val) => updateLocalField(index, "max", val)}
                              min={field.min}
                            />
                            <FieldTextarea
                              label="High Recommendation"
                              value={field.highRecommendation}
                              onChange={(val) => updateLocalField(index, "highRecommendation", val)}
                            />
                            <FieldTextarea
                              label="Normal Recommendation"
                              value={field.normalRecommendation}
                              onChange={(val) => updateLocalField(index, "normalRecommendation", val)}
                            />
                            <FieldTextarea
                              label="Low Recommendation"
                              value={field.lowRecommendation}
                              onChange={(val) => updateLocalField(index, "lowRecommendation", val)}
                            />
                          </div>
                        </div>
                      )}
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg p-6 w-full max-w-md max-h-[90vh] overflow-y-auto shadow-xl">
            <h3 className="text-xl font-bold mb-4">Add New Diet Field Definition</h3>
            <div className="space-y-4">
              <FieldInput
                label="Field Label"
                value={newFieldData.label}
                onChange={(val) => setNewFieldData({ ...newFieldData, label: val })}
              ...
              <FieldSelect
                ...
                options={categories}
                onChange={(val) => setNewFieldData({ ...newFieldData, category: val })}
              />
              <div className="grid grid-cols-2 gap-4">
                <FieldInput
                  ...
                  onChange={(val) => setNewFieldData({ ...newFieldData, min: Number(val) })}
                  min={0}
                ...
                <FieldInput
                  ...
                  onChange={(val) => setNewFieldData({ ...newFieldData, max: Number(val) })}
              ...
              <FieldTextarea
                ...
                onChange={(val) => setNewFieldData({ ...newFieldData, highRecommendation: val })}
                placeholder="Recommendation for high score"
              />
              <FieldTextarea
                label="Normal Recommendation"
                value={newFieldData.normalRecommendation}
                onChange={(val) => setNewFieldData({ ...newFieldData, normalRecommendation: val })}
                placeholder="Recommendation for normal score"
              />
              <FieldTextarea
                label="Low Recommendation"
                value={newFieldData.lowRecommendation}
                onChange={(val) => setNewFieldData({ ...newFieldData, lowRecommendation: val })}
